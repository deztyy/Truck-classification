...
def get_minio_client() -> Minio:
    ...
    try:
        ...
        # Test connection
        client.list_buckets()
        print(f"✅ MinIO connected successfully at {MINIO_ENDPOINT}!")
        return client
    except Exception as e:
        print(f"❌ MinIO connection failed: {e}")
        st.error(f"""
        ❌ ไม่สามารถเชื่อมต่อ MinIO ได้

        **ข้อผิดพลาด:** {e}

        **วิธีแก้ไข:**
        1. ตรวจสอบว่า MinIO รันอยู่หรือไม่
        2. ตรวจสอบ MINIO_ENDPOINT และ credentials ใน .env
        3. ตรวจสอบว่า {MINIO_BUCKET_NAME} bucket มีอยู่
        """)
        st.stop()
...
def get_image_from_minio(img_path: str) -> Optional[Image.Image]:
    ...
    Args:
        img_path: Path to image in MinIO (e.g., 'processed-images/vehicle_123.jpg')
    ...
    try:
        ...
        # Extract bucket and object name from img_path
        # Format: bucket_name/object_path or just object_path
        if "/" in img_path:
...
def delete_image_from_minio(img_path: str) -> bool:
    """
    Delete image from MinIO storage

    Args:
        img_path: Path to image in MinIO
...
def cleanup_old_images() -> None:
    """Delete images from MinIO that are not from today"""
    try:
        now_thailand = get_thailand_time()
        ...
        for img_path in old_images:
            if img_path and delete_image_from_minio(img_path):
                deleted_count += 1
                print(f"🗑️ Deleted old image from MinIO: {img_path}")

        if deleted_count > 0:
            print(f"✅ Cleaned up {deleted_count} old images from MinIO")
...
def render_current_vehicle_tab() -> None:
    ...
    try:
        ...
        if not df_latest.empty:
            ...
            # Display image if available from MinIO
            if (
                "img_path" in vehicle
                ...
                img = get_image_from_minio(vehicle["img_path"])
                if img:
                    st.image(img, use_container_width=True)
                else:
                    st.info("📷 Image file not found in MinIO storage")
            ...
            for _, row in df_transactions.iloc[start_idx:end_idx].iterrows():
                ...
                with st.expander(
                    ...
                    if pd.notna(row["img_path"]) and row["img_path"] != "":
                        st.markdown("---")
                        st.markdown("**📸 Vehicle Image:**")
                        img = get_image_from_minio(row["img_path"])
                        if img:
                            st.image(img, use_container_width=True)
                        else:
                            st.info("📷 Image file not found in MinIO storage")

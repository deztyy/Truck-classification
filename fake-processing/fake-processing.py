...
class ProcessingService:
    ...
    def process_task(self, task: ProcessingTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                try:
                    ...
                    # **Delete original .npy file from MinIO**
                    delete_success = self.minio_manager.delete_object(
                        bucket=task.minio_bucket, object_name=batch_object
                    )
                    
                    if delete_success:
                        logging.info(f"✓ Deleted original batch file: {batch_object}")
                    else:
                        logging.warning(f"⚠ Failed to delete batch file: {batch_object}")
        ...
        # ALWAYS clean up - runs even if there's an error
        # Clean up local file
            ...
            # ALWAYS delete .npy from MinIO after processing attempt
            if batch_object and task.minio_bucket:
                try:
                    ...
                    if delete_success:
                        logging.info(f"✓ Deleted MinIO batch file: {batch_object}")
                    else:
                        logging.warning(f"⚠ Failed to delete MinIO batch: {batch_object}")
                except Exception as e:
                    logging.error(f"✗ Error deleting MinIO batch: {e}")
